...
public class ItemController {
	...
	public String itemCreate(Item item, Model model) throws IOException, Exception {
		...
		if (count > 0) {
			model.addAttribute("message", "%s 상품 등록 성공".formatted(file.getOriginalFilename()));
			return "item/success";
		}
		model.addAttribute("message", "%s 상품 등록 실패".formatted(file.getOriginalFilename()));
		return "item/failed";
	}
	...
	public ResponseEntity<byte[]> itemDisplay(Item item) throws Exception {
		...
		try {
			// String url = e1ce17e0-7e39-4799-a8e3-459bfa0041d3_kitten-1.jpg
			...
			// 이미지파일 타입이 null이 아니라면, 헤더에 이미지타입을 저장
			if (mType != null) {
